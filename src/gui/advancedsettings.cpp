...
void AdvancedSettings::loadAdvancedSettings()
{
    ...
    item(CHECKING_MEM_USAGE, PROPERTY)->setToolTip(QString("<html><head/><body><p>%1</p></body></html>")
            .arg(tr("Higher numbers give faster rechecks but use more memory")));
    ...
    item(DISK_CACHE, PROPERTY)->setToolTip(QString("<html><head/><body><p>%1</p></body></html>")
            .arg(tr("The disk write and read cache. If this is set to -1, the cache size is automatically "
                    "set to the amount of physical RAM available in the machine divided by 8. "
                    "If this is set to 0, the disk cache is disabled. If the amount of physical RAM "
                    "cannot be determined, it's set to 16 MiB")));
    // Disk cache expiry
    ...
    item(DISK_CACHE_TTL, PROPERTY)->setToolTip(QString("<html><head/><body><p>%1</p></body></html>")
            .arg(tr("The number of seconds from the last cached write to a piece in the write cache, "
                    "to when it's forcefully flushed to disk")));
    // Enable OS cache
    ...
    item(GUIDED_READ_CACHE, PROPERTY)->setToolTip(QString("<html><head/><body><p>%1</p></body></html>")
            .arg(tr("Allows the disk cache to adjust the size of a cache line, generated by peers, "
                    "to depend on the upload rate you are sending to that peer. The intention is "
                    "to optimize the RAM usage of the cache, to read ahead further for peers that "
                    "you are sending faster to")));
    // Coalesce reads & writes
    ...
    item(COALESCE_RW, PROPERTY)->setToolTip(QString("<html><head/><body><p>%1</p></body></html>")
            .arg(tr("Allocate separate, contiguous buffers for read and write calls. "
                    "It will use more RAM but may improve performance")));
#endif
    // Suggest mode
    ...
    item(SUGGEST_MODE, PROPERTY)->setToolTip(QString("<html><head/><body><p>%1</p></body></html>")
            .arg(tr("Send out suggest messages to create a bias for peers to request certain pieces")));
    // Send buffer watermark
    ...
    item(SEND_BUF_WATERMARK, PROPERTY)->setToolTip(QString("<html><head/><body><p>%1</p></body></html>")
            .arg(tr("This is the upper limit for the send buffer size. If the send buffer has fewer bytes "
                    "than \"Send buffer watermark\", we'll read another 16kB block onto it. If set too small, "
                    "upload rate capacity will suffer. If set too high, memory will be wasted. The actual watermark "
                    "may be lower than this in case the upload rate is low")));
    ...
    item(SEND_BUF_LOW_WATERMARK, PROPERTY)->setToolTip(QString("<html><head/><body><p>%1</p></body></html>")
            .arg(tr("The minimum send buffer target size (send buffer includes bytes pending being read from disk). "
    ...
    item(SEND_BUF_WATERMARK_FACTOR, PROPERTY)->setToolTip(QString("<html><head/><body><p>%1</p></body></html>")
            .arg(tr("The factor is specified as a percentage. i.e. 50 -> 0.5. The current upload rate to a peer is "
                    "multiplied by this factor to get the send buffer watermark. This product is clamped to "
                    "the \"Send buffer watermark\" setting so as to not exceed the max. For high speed upload, this "
                    "should be set to a greater value than 100. For high capacity connections, setting this higher "
                    "can improve upload performance and disk throughput. Setting it too high may waste RAM and "
    ...
    item(SUPER_SEEDING, PROPERTY)->setToolTip(QString("<html><head/><body><p>%1</p></body></html>")
            .arg(tr("If this is set to true, peers must forward received pieces before being sent new ones")));
    // Network interface
    ...
    item(SEED_CHOKING_ALGORITHM, PROPERTY)->setToolTip(QString("<html><head/><body><p>%1</p></body></html>")
            .arg(tr("Controls the seeding unchoke behavior. The available options are:<br><br>"
                    "<strong>Round-robin</strong>: rotates the peers that are unchoked when seeding. "
                    "This distributes the upload bandwidth uniformly and fairly. It minimizes the ability"
                    " for a peer to download everything without redistributing it<br><br>"
                    "<strong>Fastest upload</strong>: unchokes the peers we can send to the fastest. This might be a bit "
                    "more reliable in utilizing all available capacity<br><br>"
                    "<strong>Anti-leech</strong>: prioritizes peers who have just started or are just about to finish the download. "
