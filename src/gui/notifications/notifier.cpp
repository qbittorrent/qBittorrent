...
namespace {
    inline QString torrentNameOrDefault(const BitTorrent::TorrentHandle* h)
    {
        return h ? h->name() : QObject::tr("Torrent");
    }
}
...
void Notifier::notifyTorrentFinished(const BitTorrent::TorrentHandle *h, QWidget *widget)
{
    ...
    showNotification(Notifier::TorrentFinished, tr("Download completion"),
                     tr("%1 has finished downloading.", "e.g: xxx.avi has finished downloading.")
                        .arg(torrentNameOrDefault(h)), &ct);
}

void Notifier::notifyTorrentIOError(const BitTorrent::TorrentHandle *h, const QString &message, QWidget *widget)
{
    Context ct(widget);
    showNotification(Notifier::TorrentIOError, tr("I/O Error", "i.e: Input/Output Error"),
                     tr("An I/O error occurred for torrent %1.\n Reason: %2",
                        ...
                        .arg(torrentNameOrDefault(h)).arg(message), &ct);
}
