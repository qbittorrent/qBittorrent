...
void PropertiesWidget::loadTorrentInfos(BitTorrent::Torrent *const torrent)
{
    ...
    if (m_torrent->hasMetadata())
    {
        ...
        m_ui->contentWidget->setContentTorrent(m_torrent);
    }
    // Load dynamic data
    loadDynamicData();
}
...
void PropertiesWidget::loadDynamicData()
{
    ...
    case PropTabBar::FilesTab:
        ...
        if (m_torrent->hasMetadata())
        {
            qDebug("Updating progress and availability in files tab");
            m_ui->contentWidget->loadDynamicData();
        }
        break;
    default:;
    }
