    ...
    , m_torrent(nullptr)
{
    ...
    m_ui->trackerUpButton->setIcon(UIThemeManager::instance()->getIcon("TrackerList.UpAction"));
    m_ui->trackerUpButton->setIconSize(Utils::Gui::smallIconSize());
    m_ui->trackerDownButton->setIcon(UIThemeManager::instance()->getIcon("TrackerList.DownAction"));
...
void PropertiesWidget::displayFilesListMenu(const QPoint &)
{
    ...
    if (selectedRows.size() == 1) {
        ...
        const QAction *actOpen = menu->addAction(UIThemeManager::instance()->getIcon("TorrentContent.OpenAction"), tr("Open"));
        connect(actOpen, &QAction::triggered, this, [this, index]() { openDoubleClickedFile(index); });

        const QAction *actOpenContainingFolder = menu->addAction(UIThemeManager::instance()->getIcon("TorrentContent.ContainingFolder.OpenAction"), tr("Open Containing Folder"));
        connect(actOpenContainingFolder, &QAction::triggered, this, [this, index]() { openFolder(index, true); });

        const QAction *actRename = menu->addAction(UIThemeManager::instance()->getIcon("TorrentContent.RenameAction"), tr("Rename..."));
...
void PropertiesWidget::displayWebSeedListMenu(const QPoint &)
{
    ...
    const QAction *actAdd = menu->addAction(UIThemeManager::instance()->getIcon("WebSeed.AddAction"), tr("New Web seed"));
    connect(actAdd, &QAction::triggered, this, &PropertiesWidget::askWebSeed);

    if (!rows.isEmpty()) {
        const QAction *actDel = menu->addAction(UIThemeManager::instance()->getIcon("WebSeed.RemoveAction"), tr("Remove Web seed"));
        connect(actDel, &QAction::triggered, this, &PropertiesWidget::deleteSelectedUrlSeeds);

        menu->addSeparator();

        const QAction *actCpy = menu->addAction(UIThemeManager::instance()->getIcon("WebSeedURL.CopyAction"), tr("Copy Web seed URL"));
        connect(actCpy, &QAction::triggered, this, &PropertiesWidget::copySelectedWebSeedsToClipboard);

        const QAction *actEdit = menu->addAction(UIThemeManager::instance()->getIcon("WebSeedURL.EditAction"), tr("Edit Web seed URL"));
