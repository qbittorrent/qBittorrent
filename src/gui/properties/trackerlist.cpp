...
void TrackerList::loadTrackers() {
  ...
  foreach (const BitTorrent::TrackerEntry &entry, torrent->trackerEntries()) {
...
void TrackerList::askForTrackers() {
  ...
  QStringList trackers = TrackersAdditionDlg::askForTrackers(torrent);
...
void TrackerList::deleteSelectedTrackers() {
  ...
  QList<BitTorrent::TrackerEntry> trackers = torrent->trackerEntries();
  ...
  foreach (const QString &url, urls_to_remove)
      emit trackerRemoved(url, hash);
  if (remaining_trackers.empty())
      emit trackerlessStateChanged(true, hash);
  if (!torrent->isPaused())
    torrent->forceReannounce();
  // Reload Trackers
  loadTrackers();
...
void TrackerList::reannounceSelected() {
    ...
    QStringList trackers = torrent->trackers();
    for (int i = 0; i < trackers.size(); ++i) {
      foreach (QTreeWidgetItem* w, selected_items) {
        if (w->text(COL_URL) == trackers[i]) {
          torrent->forceReannounce(0, i);
          break;
        }
      }
    }
