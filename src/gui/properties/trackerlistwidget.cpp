...
void TrackerListWidget::showTrackerListMenu(const QPoint &)
{
    ...
    // Add actions
    const QAction *addAct = menu->addAction(UIThemeManager::instance()->getIcon("TrackerList.AddAction"), tr("Add a new tracker..."));
    connect(addAct, &QAction::triggered, this, &TrackerListWidget::askForTrackers);

    if (!getSelectedTrackerItems().isEmpty()) {
        const QAction *editAct = menu->addAction(UIThemeManager::instance()->getIcon("TrackerList.URL.EditAction"),tr("Edit tracker URL..."));
        connect(editAct, &QAction::triggered, this, &TrackerListWidget::editSelectedTracker);

        const QAction *delAct = menu->addAction(UIThemeManager::instance()->getIcon("TrackerList.RemoveAction"), tr("Remove tracker"));
        connect(delAct, &QAction::triggered, this, &TrackerListWidget::deleteSelectedTrackers);

        const QAction *copyAct = menu->addAction(UIThemeManager::instance()->getIcon("TrackerList.URL.CopyAction"), tr("Copy tracker URL"));
        connect(copyAct, &QAction::triggered, this, &TrackerListWidget::copyTrackerUrl);
    }

    if (!torrent->isPaused()) {
        const QAction *reannounceSelAct = menu->addAction(UIThemeManager::instance()->getIcon("TrackerList.ReAnnounce.RefreshAction"), tr("Force reannounce to selected trackers"));
        connect(reannounceSelAct, &QAction::triggered, this, &TrackerListWidget::reannounceSelected);

        menu->addSeparator();

        const QAction *reannounceAllAct = menu->addAction(UIThemeManager::instance()->getIcon("TrackerList.ReAnnounceAll.RefreshAction"), tr("Force reannounce to all trackers"));
