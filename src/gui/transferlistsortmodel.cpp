...
bool TransferListSortModel::lessThan(const QModelIndex &left, const QModelIndex &right) const
{
    ...
    if (column == TorrentModelItem::TR_NAME) {
        ...
        bool activeR = TorrentFilter::ActiveTorrent.match(
                    model->data(model->index(right.row(), TorrentModelItem::TR_STATUS)).toInt(),
                    model->data(model->index(right.row(), TorrentModelItem::TR_UPSPEED)).toInt());
        bool activeL = TorrentFilter::ActiveTorrent.match(
                    model->data(model->index(left.row(), TorrentModelItem::TR_STATUS)).toInt(),
                    model->data(model->index(left.row(), TorrentModelItem::TR_UPSPEED)).toInt());
...
bool TransferListSortModel::matchFilter(int sourceRow, const QModelIndex &sourceParent) const
{
    ...
    BitTorrent::TorrentState state = model->index(sourceRow, TorrentModelItem::TR_STATUS, sourceParent).data().toInt();
    int upSpeed = model->index(sourceRow, TorrentModelItem::TR_UPSPEED, sourceParent).data().toInt();
    QString hash = model->torrentHash(sourceRow);
    QString label = model->index(sourceRow, TorrentModelItem::TR_LABEL, sourceParent).data().toString();

    return m_filter.match(state, upSpeed, hash, label);
}
