...
void TransferListWidget::torrentDoubleClicked()
{
    ...
    if (torrent->isFinished())
        action = qBt->preferences()->getActionOnDblClOnTorrentFn();
    else
        action = qBt->preferences()->getActionOnDblClOnTorrentDl();
...
{
    if (qBt->preferences()->confirmPauseAndResumeAll())
    {
    ...
    for (BitTorrent::Torrent *const torrent : asConst(qBt->btSession()->torrents()))
        torrent->pause();
}
...
void TransferListWidget::deleteSelectedTorrents(const bool deleteLocalFiles)
{
    ...
    if (qBt->preferences()->confirmTorrentDeletion())
    {
...
void TransferListWidget::increaseQueuePosSelectedTorrents()
{
    ...
    if (m_mainWindow->currentTabWidget() == this)
        qBt->btSession()->increaseTorrentsQueuePos(extractIDs(getSelectedTorrents()));
}
...
void TransferListWidget::decreaseQueuePosSelectedTorrents()
{
    qDebug() << Q_FUNC_INFO;
    if (m_mainWindow->currentTabWidget() == this)
        qBt->btSession()->decreaseTorrentsQueuePos(extractIDs(getSelectedTorrents()));
}

void TransferListWidget::topQueuePosSelectedTorrents()
{
    if (m_mainWindow->currentTabWidget() == this)
        qBt->btSession()->topTorrentsQueuePos(extractIDs(getSelectedTorrents()));
}

void TransferListWidget::bottomQueuePosSelectedTorrents()
{
    if (m_mainWindow->currentTabWidget() == this)
        qBt->btSession()->bottomTorrentsQueuePos(extractIDs(getSelectedTorrents()));
}
...
{
    if (qBt->preferences()->confirmTorrentRecheck())
    {
...
void TransferListWidget::displayColumnHeaderMenu()
{
    ...
    for (int i = 0; i < TransferListModel::NB_COLUMNS; ++i)
    {
        if (!qBt->btSession()->isQueueingSystemEnabled() && (i == TransferListModel::TR_QUEUE_POSITION))
            continue;
...
void TransferListWidget::displayListMenu()
{
    ...
    auto *actionStart = new QAction(qBt->uiThemeManager()->getIcon(u"torrent-start"_s, u"media-playback-start"_s), tr("&Resume", "Resume/start the torrent"), listMenu);
    connect(actionStart, &QAction::triggered, this, &TransferListWidget::startSelectedTorrents);
    auto *actionPause = new QAction(qBt->uiThemeManager()->getIcon(u"torrent-stop"_s, u"media-playback-pause"_s), tr("&Pause", "Pause the torrent"), listMenu);
    connect(actionPause, &QAction::triggered, this, &TransferListWidget::pauseSelectedTorrents);
    auto *actionForceStart = new QAction(qBt->uiThemeManager()->getIcon(u"torrent-start-forced"_s, u"media-playback-start"_s), tr("Force Resu&me", "Force Resume/start the torrent"), listMenu);
    connect(actionForceStart, &QAction::triggered, this, &TransferListWidget::forceStartSelectedTorrents);
    auto *actionDelete = new QAction(qBt->uiThemeManager()->getIcon(u"list-remove"_s), tr("&Remove", "Remove the torrent"), listMenu);
    connect(actionDelete, &QAction::triggered, this, &TransferListWidget::softDeleteSelectedTorrents);
    auto *actionPreviewFile = new QAction(qBt->uiThemeManager()->getIcon(u"view-preview"_s), tr("Pre&view file..."), listMenu);
    connect(actionPreviewFile, &QAction::triggered, this, &TransferListWidget::previewSelectedTorrents);
    auto *actionTorrentOptions = new QAction(qBt->uiThemeManager()->getIcon(u"configure"_s), tr("Torrent &options..."), listMenu);
    connect(actionTorrentOptions, &QAction::triggered, this, &TransferListWidget::setTorrentOptions);
    auto *actionOpenDestinationFolder = new QAction(qBt->uiThemeManager()->getIcon(u"directory"_s), tr("Open destination &folder"), listMenu);
    connect(actionOpenDestinationFolder, &QAction::triggered, this, &TransferListWidget::openSelectedTorrentsFolder);
    auto *actionIncreaseQueuePos = new QAction(qBt->uiThemeManager()->getIcon(u"go-up"_s), tr("Move &up", "i.e. move up in the queue"), listMenu);
    connect(actionIncreaseQueuePos, &QAction::triggered, this, &TransferListWidget::increaseQueuePosSelectedTorrents);
    auto *actionDecreaseQueuePos = new QAction(qBt->uiThemeManager()->getIcon(u"go-down"_s), tr("Move &down", "i.e. Move down in the queue"), listMenu);
    connect(actionDecreaseQueuePos, &QAction::triggered, this, &TransferListWidget::decreaseQueuePosSelectedTorrents);
    auto *actionTopQueuePos = new QAction(qBt->uiThemeManager()->getIcon(u"go-top"_s), tr("Move to &top", "i.e. Move to top of the queue"), listMenu);
    connect(actionTopQueuePos, &QAction::triggered, this, &TransferListWidget::topQueuePosSelectedTorrents);
    auto *actionBottomQueuePos = new QAction(qBt->uiThemeManager()->getIcon(u"go-bottom"_s), tr("Move to &bottom", "i.e. Move to bottom of the queue"), listMenu);
    connect(actionBottomQueuePos, &QAction::triggered, this, &TransferListWidget::bottomQueuePosSelectedTorrents);
    auto *actionSetTorrentPath = new QAction(qBt->uiThemeManager()->getIcon(u"set-location"_s, u"inode-directory"_s), tr("Set loc&ation..."), listMenu);
    connect(actionSetTorrentPath, &QAction::triggered, this, &TransferListWidget::setSelectedTorrentsLocation);
    auto *actionForceRecheck = new QAction(qBt->uiThemeManager()->getIcon(u"force-recheck"_s, u"document-edit-verify"_s), tr("Force rec&heck"), listMenu);
    connect(actionForceRecheck, &QAction::triggered, this, &TransferListWidget::recheckSelectedTorrents);
    auto *actionForceReannounce = new QAction(qBt->uiThemeManager()->getIcon(u"reannounce"_s, u"document-edit-verify"_s), tr("Force r&eannounce"), listMenu);
    connect(actionForceReannounce, &QAction::triggered, this, &TransferListWidget::reannounceSelectedTorrents);
    auto *actionCopyMagnetLink = new QAction(qBt->uiThemeManager()->getIcon(u"torrent-magnet"_s, u"kt-magnet"_s), tr("&Magnet link"), listMenu);
    connect(actionCopyMagnetLink, &QAction::triggered, this, &TransferListWidget::copySelectedMagnetURIs);
    auto *actionCopyID = new QAction(qBt->uiThemeManager()->getIcon(u"help-about"_s, u"edit-copy"_s), tr("Torrent &ID"), listMenu);
    connect(actionCopyID, &QAction::triggered, this, &TransferListWidget::copySelectedIDs);
    auto *actionCopyName = new QAction(qBt->uiThemeManager()->getIcon(u"name"_s, u"edit-copy"_s), tr("&Name"), listMenu);
    connect(actionCopyName, &QAction::triggered, this, &TransferListWidget::copySelectedNames);
    auto *actionCopyHash1 = new QAction(qBt->uiThemeManager()->getIcon(u"hash"_s, u"edit-copy"_s), tr("Info &hash v1"), listMenu);
    connect(actionCopyHash1, &QAction::triggered, this, [this]() { copySelectedInfohashes(CopyInfohashPolicy::Version1); });
    auto *actionCopyHash2 = new QAction(qBt->uiThemeManager()->getIcon(u"hash"_s, u"edit-copy"_s), tr("Info h&ash v2"), listMenu);
    ...
    auto *actionRename = new QAction(qBt->uiThemeManager()->getIcon(u"edit-rename"_s), tr("Re&name..."), listMenu);
    ...
    auto *actionEditTracker = new QAction(qBt->uiThemeManager()->getIcon(u"edit-rename"_s), tr("Edit trac&kers..."), listMenu);
    connect(actionEditTracker, &QAction::triggered, this, &TransferListWidget::editTorrentTrackers);
    auto *actionExportTorrent = new QAction(qBt->uiThemeManager()->getIcon(u"edit-copy"_s), tr("E&xport .torrent..."), listMenu);
    ...
    for (const QModelIndex &index : selectedIndexes)
    {
        ...
        const bool queued = (qBt->btSession()->isQueueingSystemEnabled() && torrent->isQueued());
    ...
    QMenu *categoryMenu = listMenu->addMenu(qBt->uiThemeManager()->getIcon(u"view-categories"_s), tr("Categor&y"));

    categoryMenu->addAction(qBt->uiThemeManager()->getIcon(u"list-add"_s), tr("&New...", "New category...")
        , this, &TransferListWidget::askNewCategoryForSelection);
    categoryMenu->addAction(qBt->uiThemeManager()->getIcon(u"edit-clear"_s), tr("&Reset", "Reset category")
    ...
    for (const QString &category : asConst(categories))
    {
        ...
        QAction *categoryAction = categoryMenu->addAction(qBt->uiThemeManager()->getIcon(u"view-categories"_s), escapedCategory
    ...
    // Tag Menu
    QStringList tags(qBt->btSession()->tags().values());
    std::sort(tags.begin(), tags.end(), Utils::Compare::NaturalLessThan<Qt::CaseInsensitive>());

    QMenu *tagsMenu = listMenu->addMenu(qBt->uiThemeManager()->getIcon(u"tags"_s, u"view-categories"_s), tr("Ta&gs"));

    tagsMenu->addAction(qBt->uiThemeManager()->getIcon(u"list-add"_s), tr("&Add...", "Add / assign multiple tags...")
        , this, &TransferListWidget::askAddTagsForSelection);
    tagsMenu->addAction(qBt->uiThemeManager()->getIcon(u"edit-clear"_s), tr("&Remove All", "Remove all tags")
        , this, [this]()
    {
        if (qBt->preferences()->confirmRemoveAllTags())
    ...
    if (qBt->btSession()->isQueueingSystemEnabled() && oneNotFinished)
    {
        listMenu->addSeparator();
        QMenu *queueMenu = listMenu->addMenu(
            qBt->uiThemeManager()->getIcon(u"queued"_s), tr("&Queue"));
    ...
    QMenu *copySubMenu = listMenu->addMenu(qBt->uiThemeManager()->getIcon(u"edit-copy"_s), tr("&Copy"));
...
void TransferListWidget::applyFilter(const QString &name, const TransferListModel::Column &type)
{
    ...
    const QString pattern = (qBt->preferences()->getRegexAsFilteringPatternForTransferList()
...
{
    qBt->preferences()->setTransHeaderState(header()->saveState());
}

bool TransferListWidget::loadSettings()
{
    return header()->restoreState(qBt->preferences()->getTransHeaderState());
}
