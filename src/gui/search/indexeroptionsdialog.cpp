    ...
    , m_storeDialogSize {SETTINGS_KEY("DialogSize")}
{
    ...
    const QRegularExpression namePattern {QLatin1String("\\S.*")};
    m_ui->lineAPIKey->setValidator(new QRegularExpressionValidator(namePattern, this));

    const QRegularExpression urlPattern {QLatin1String("\\S.*")};
    m_ui->lineAPIKey->setValidator(new QRegularExpressionValidator(urlPattern, this));

    const QRegularExpression apiKeyPattern {QLatin1String("\\S.*")};
    m_ui->lineAPIKey->setValidator(new QRegularExpressionValidator(apiKeyPattern, this));
...
void IndexerOptionsDialog::validate()
{
    m_ui->buttonBox->button(QDialogButtonBox::Ok)->setEnabled(
                m_ui->lineName->hasAcceptableInput()
                && m_ui->lineURL->hasAcceptableInput()
                && m_ui->lineAPIKey->hasAcceptableInput());
}
