...
QVariant TorrentModelItem::data(int column, int role) const
{
    ...
    case TR_AMOUNT_LEFT:
        return m_torrent->uncompletedSize();
    case TR_TIME_ELAPSED:
        return (role == Qt::DisplayRole) ? m_torrent->activeTime() : m_torrent->seedingTime();
    case TR_SAVE_PATH:
        return fsutils::toNativePath(m_torrent->savePathParsed());
    case TR_COMPLETED:
...
void TorrentModel::populate()
{
    ...
    connect(BitTorrent::Session::instance(), SIGNAL(metadataLoaded(BitTorrent::TorrentHandle *const)), SLOT(handleTorrentStatusUpdated(BitTorrent::TorrentHandle *const)));
...
TorrentStatusReport TorrentModel::getTorrentStatusReport() const
{
    ...
    foreach (TorrentModelItem *const item, m_items) {
        if (TorrentFilter::DownloadingTorrent.match(item->torrentHandle()))
            ++report.nb_downloading;
        if (TorrentFilter::SeedingTorrent.match(item->torrentHandle()))
            ++report.nb_seeding;
        if (TorrentFilter::CompletedTorrent.match(item->torrentHandle()))
            ++report.nb_completed;
        if (TorrentFilter::PausedTorrent.match(item->torrentHandle()))
            ++report.nb_paused;
        if (TorrentFilter::ResumedTorrent.match(item->torrentHandle()))
            ++report.nb_resumed;
        if (TorrentFilter::ActiveTorrent.match(item->torrentHandle()))
            ++report.nb_active;
        if (TorrentFilter::InactiveTorrent.match(item->torrentHandle()))
            ++report.nb_inactive;
    }
