...
void sendProgressUpdateSignal(int i, int num, TorrentCreatorThread *sender)
{
    sender->sendProgressSignal((int)((i * 100.) / num));
}
...
void TorrentCreatorThread::create(QString inputPath, QString savePath, QStringList trackers, QStringList urlSeeds, QString comment, bool isPrivate, int pieceSize)
{
...
void TorrentCreatorThread::run()
{
    ...
    try {
        ...
        libt::add_files(fs, fsutils::toNativePath(m_inputPath).toUtf8().constData(), fileFilter);
        if (m_abort) return;
        ...
        libt::set_piece_hashes(t, fsutils::toNativePath(parentPath).toUtf8().constData(), boost::bind(sendProgressUpdateSignal, _1, t.num_pieces(), this));
