...
{
public:
    explicit TorrentInfo(const libtorrent::torrent_info *nativeInfo = 0);
    ...
    QString filePath(uint index) const;
    QStringList filePaths() const;
    QString fileName(uint index) const;
    QString origFilePath(uint index) const;
    qlonglong fileSize(uint index) const;
...
private:
    QSharedPointer<libtorrent::torrent_info> m_nativeInfo;
};
