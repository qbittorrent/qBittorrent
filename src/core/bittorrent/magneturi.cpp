    ...
    , m_url(url)
{
    ...
    m_name = QString::fromUtf8(m_addTorrentParams.name.c_str());

    foreach (const std::string &tracker, m_addTorrentParams.trackers)
        m_trackers.append(QString::fromUtf8(tracker.c_str()));

#if LIBTORRENT_VERSION_NUM >= 10000
    foreach (const std::string &urlSeed, m_addTorrentParams.url_seeds)
        m_urlSeeds.append(QString::fromUtf8(urlSeed.c_str()));
#endif
}
...
QStringList MagnetUri::trackers() const
{
    return m_trackers;
}

QStringList MagnetUri::urlSeeds() const
{
    return m_urlSeeds;
}
