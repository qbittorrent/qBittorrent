...
window.qBittorrent.TorrentContent ??= (() => {
    ...
    const triStateFromPriority = function(priority) {
    ...
    const getCheckboxState = function(id) {
        const row = torrentFilesTable.rows.get(id.toString());
    ...
    const filesPriorityMenuClicked = function(priority) {
        ...
        selectedRows.forEach((rowId) => {
            ...
            rowIds.push(rowId);
            fileIds.push(parseInt(elem.getAttribute("data-file-id"), 10));
        });
    ...
    const clearFilterInputTimer = function() {
        clearTimeout(torrentFilesFilterInputTimer);
        torrentFilesFilterInputTimer = -1;
    };
