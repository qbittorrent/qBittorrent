...
window.qBittorrent.PiecesBar ??= (() => {
     ...
     */
    class PiecesBarBase extends HTMLElement {
        _canvasEl;
        _ctx;
        _styles;
        #id = ++piecesBarUniqueId;
        #resizeObserver;
        ...
        _refresh() {
            ...
            const width = Math.min(this.offsetWidth, data.length, MAX_CANVAS_WIDTH);
        ...
        _getData() { throw new Error("_getData must be implemented"); }
        _shouldRender(imageWidth) { throw new Error("_shouldRender must be implemented"); }
        _computePixelValue(piecesFrom, piecesTo, ratio) { throw new Error("_computePixelValue must be implemented"); }
        _valuesEqual(a, b) { throw new Error("_valuesEqual must be implemented"); }
        _drawRectangle(start, width, value) { throw new Error("_drawRectangle must be implemented"); }
    }
     ...
     */
    class PiecesBar extends PiecesBarBase {
        ...
        _valuesEqual(a, b) {
            return (a[PiecesBar.#STATUS_DOWNLOADING] === b[PiecesBar.#STATUS_DOWNLOADING])
                && (a[PiecesBar.#STATUS_DOWNLOADED] === b[PiecesBar.#STATUS_DOWNLOADED]);
        }
     ...
     */
    class AvailabilityBar extends PiecesBarBase {
        #availability = [];
        #maxAvailability = 0;
        ...
        setAvailability(availability) {
            ...
            this.#maxAvailability = 0;
            for (const avail of this.#availability) {
                if (avail > this.#maxAvailability)
                    this.#maxAvailability = avail;
            }
            this._refresh();
        }
