...
void SearchController::startAction()
{
    ...
    const auto searchResult = m_searchJobManager->startSearch(pattern, category, pluginsToUse);
    if (!searchResult)
        throw APIError(APIErrorType::Conflict, searchResult.error());

    const QJsonObject result = {{u"id"_s, *searchResult}};
    setResult(result);
}
