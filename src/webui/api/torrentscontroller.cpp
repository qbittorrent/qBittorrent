    ...
    void applyToTorrents(const QStringList &idList, const std::function<void (BitTorrent::Torrent *torrent)> &func)
    {
        ...
        else
        {
            for (const QString &idString : idList)
            {
                ...
                BitTorrent::Torrent *const torrent = BitTorrent::Session::instance()->getTorrentByAnyHashForm(hash);
                if (torrent)
                    func(torrent);
            }
        }
...
void TorrentsController::trackersAction()
{
    ...
    const auto id = BitTorrent::TorrentID::fromString(params()[u"hash"_qs]);
    const BitTorrent::Torrent *const torrent = BitTorrent::Session::instance()->getTorrentByAnyHashForm(id);
    if (!torrent)
        throw APIError(APIErrorType::NotFound);
...
void TorrentsController::uploadLimitAction()
{
    ...
    {
        int limit = -1;
        const BitTorrent::Torrent *const torrent = BitTorrent::Session::instance()->getTorrentByAnyHashForm(BitTorrent::TorrentID::fromString(id));
        if (torrent)
