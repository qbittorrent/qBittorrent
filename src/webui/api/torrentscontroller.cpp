...
void TorrentsController::addAction()
{
    ...
    for (QString url : urls)
    {
        ...
        if (const BitTorrent::TorrentDescriptor &torrentDescr = m_torrentMetadataCache[infoHash]; torrentDescr.info().has_value())
        {
...
void TorrentsController::fetchMetadataAction()
{
    ...
    if (sourceTorrentDescr)
        infoHash = sourceTorrentDescr.value().infoHash();
    else if (const auto iter = m_torrentSourceCache.constFind(source); iter != m_torrentSourceCache.constEnd())
        infoHash = iter.value();
    ...
    {
        if (!m_torrentSourceCache.contains(source) && !m_requestedTorrentSource.contains(source))
        {
...
void TorrentsController::saveMetadataAction()
{
    ...
    const BitTorrent::TorrentDescriptor &torrentDescr = m_torrentMetadataCache[infoHash];
