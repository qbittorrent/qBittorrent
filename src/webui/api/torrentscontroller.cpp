...
namespace
{
    ...
    const QStringList SUPPORTED_WEB_SEED_SCHEMES {u"http"_s, u"https"_s, u"ftp"_s};
    ...
    nonstd::expected<QUrl, QString> validateWebSeedUrl(const QString &urlStr)
    {
        ...
        if (!SUPPORTED_WEB_SEED_SCHEMES.contains(url.scheme()))
            return nonstd::make_unexpected(TorrentsController::tr("URL scheme must be one of [%1]").arg(SUPPORTED_WEB_SEED_SCHEMES.join(u",")));
...
void TorrentsController::editWebSeedAction()
{
    ...
    if (newUrl != origUrl)
    {
        if (!torrent->urlSeeds().contains(origUrl))
            throw APIError(APIErrorType::BadParams, tr("\"%1\" is not an existing url").arg(origUrl.toString()));
