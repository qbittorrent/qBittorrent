...
bool SearchJobManager::deleteSearch(const int id)
{
    ...
    if (const auto iter = m_restoredJobs.constFind(id); iter != m_restoredJobs.cend())
    {
        m_restoredJobs.erase(iter);
        found = true;
    }
...
std::optional<SearchJobManager::JobInfo> SearchJobManager::getJobInfo(const int id) const
{
    if (const auto iter = m_searchHandlers.constFind(id); iter != m_searchHandlers.cend())
    {
        ...
        {
            .id = id,
            .pattern = iter.value()->pattern(),
            .active = m_activeSearches.contains(id),
            .results = iter.value()->results()
        };
    }
...
int SearchJobManager::generateSearchId() const
{
    while (true)
    {
        ...
        if (!m_searchHandlers.contains(id) && !m_restoredJobs.contains(id))
            return id;
    }
}
...
void SearchJobManager::loadSession()
{
    ...
    for (const QJsonValue &tabValue : tabsArray)
    {
        ...
        m_restoredJobs.insert(tabId, jobInfo);
...
void SearchJobManager::saveSession() const
{
    ...
    {
        QString pattern;
        if (const auto iter = m_searchHandlers.constFind(searchId); iter != m_searchHandlers.cend())
            pattern = iter.value()->pattern();
        else if (const auto iter = m_restoredJobs.constFind(searchId); iter != m_restoredJobs.cend())
            pattern = iter.value().pattern;
