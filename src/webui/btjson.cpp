...
static QVariantMap toMap(const QTorrentHandle& h)
{
  ...
  ret[KEY_TORRENT_DLSPEED] = misc::friendlyUnit(h.download_payload_rate(), true); // FIXME: Should be passed as a Number
  ret[KEY_TORRENT_UPSPEED] = misc::friendlyUnit(h.upload_payload_rate(), true); // FIXME: Should be passed as a Number
...
QByteArray btjson::getPropertiesForTorrent(const QString& hash)
{
  ...
  try {
    ...
    data[KEY_PROP_WASTED] = misc::friendlyUnit(h.total_failed_bytes() + h.total_redundant_bytes());
    data[KEY_PROP_UPLOADED] = QString(misc::friendlyUnit(h.all_time_upload()) + " (" + misc::friendlyUnit(h.total_payload_upload()) + " " + tr("this session") + ")");
    data[KEY_PROP_DOWNLOADED] = QString(misc::friendlyUnit(h.all_time_download()) + " (" + misc::friendlyUnit(h.total_payload_download()) + " " + tr("this session") + ")");
    ...
    data[KEY_PROP_CONNECT_COUNT] = QString(QString::number(h.num_connections()) + " (" + tr("%1 max", "e.g. 10 max").arg(QString::number(h.connections_limit())) + ")");
    const qreal ratio = QBtSession::instance()->getRealRatio(h.hash());
