...
void WebApplication::setSessionCookie()
{
    ...
    cookie.setExpirationDate(QDateTime::currentDateTime().addDuration(m_sessionTimeout));
    ...
    m_currentSession->setCookieRefreshTime(m_sessionTimeout);
}
...
{
    return m_timestamp.hasExpired(duration.count());
}
