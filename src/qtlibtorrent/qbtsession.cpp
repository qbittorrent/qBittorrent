  ...
  while (a.get()) { // while alert available
    try {
      ...
      case external_ip_alert::alert_type:
        externalIpAlert(static_cast<external_ip_alert*>(a.get()));
        break;
      }
...
void QBtSession::torrentFinishedAlert(libtorrent::torrent_finished_alert *p)
{
...
void QBtSession::saveResumeDataAlert(libtorrent::save_resume_data_alert *p)
{
  const QDir torrentBackup(fsutils::BTBackupLocation());
...
void QBtSession::fileRenamedAlert(libtorrent::file_renamed_alert *p)
{
  QTorrentHandle h(p->handle);
  if (h.is_valid()) {
    if (h.num_files() > 1) {
...
void QBtSession::torrentDeletedAlert(libtorrent::torrent_deleted_alert *p)
{
...
void QBtSession::storageMovedAlert(libtorrent::storage_moved_alert *p)
{
  QTorrentHandle h(p->handle);
  if (h.is_valid()) {
...
void QBtSession::metadataReceivedAlert(libtorrent::metadata_received_alert *p)
{
  QTorrentHandle h(p->handle);
  Preferences pref;
  if (h.is_valid()) {
...
void QBtSession::fileErrorAlert(libtorrent::file_error_alert *p)
{
  QTorrentHandle h(p->handle);
  if (h.is_valid()) {
    h.pause();
...
void QBtSession::fileCompletedAlert(libtorrent::file_completed_alert *p)
{
  QTorrentHandle h(p->handle);
...
void QBtSession::torrentPausedAlert(libtorrent::torrent_paused_alert *p)
{
...
void QBtSession::trackerErrorAlert(libtorrent::tracker_error_alert *p)
{
  // Level: fatal
  QTorrentHandle h(p->handle);
  if (h.is_valid()) {
...
void QBtSession::trackerReplyAlert(libtorrent::tracker_reply_alert *p)
{
  const QTorrentHandle h(p->handle);
  if (h.is_valid()) {
...
void QBtSession::trackerWarningAlert(libtorrent::tracker_warning_alert *p)
{
  const QTorrentHandle h(p->handle);
  if (h.is_valid()) {
...
void QBtSession::portmapErrorAlert(libtorrent::portmap_error_alert *p)
{
...
void QBtSession::portmapAlert(libtorrent::portmap_alert *p)
{
...
void QBtSession::peerBlockedAlert(libtorrent::peer_blocked_alert *p)
{
...
void QBtSession::peerBanAlert(libtorrent::peer_ban_alert *p)
{
  boost::system::error_code ec;
...
void QBtSession::fastresumeRejectedAlert(libtorrent::fastresume_rejected_alert *p)
{
  QTorrentHandle h(p->handle);
  if (h.is_valid()) {
...
void QBtSession::urlSeedAlert(libtorrent::url_seed_alert *p)
{
...
void QBtSession::listenSucceededAlert(libtorrent::listen_succeeded_alert *p)
{
  boost::system::error_code ec;
  QString proto = "TCP";
#if LIBTORRENT_VERSION_NUM >= 10000
...
void QBtSession::listenFailedAlert(libtorrent::listen_failed_alert* p)
{
  boost::system::error_code ec;
  QString proto = "TCP";
#if LIBTORRENT_VERSION_NUM >= 10000
...
void QBtSession::torrentCheckedAlert(libtorrent::torrent_checked_alert* p)
{
  QTorrentHandle h(p->handle);
  if (h.is_valid()) {
    const QString hash = h.hash();
...
void QBtSession::externalIpAlert(libtorrent::external_ip_alert* p)
{
  boost::system::error_code ec;
