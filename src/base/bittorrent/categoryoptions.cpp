...
BitTorrent::CategoryOptions BitTorrent::CategoryOptions::fromJSON(const QJsonObject &jsonObj)
{
    ...
    options.ratioLimit = jsonObj.value(OPTION_RATIOLIMIT).toDouble(Torrent::DEFAULT_RATIO_LIMIT);
    options.seedingTimeLimit = jsonObj.value(OPTION_SEEDINGTIMELIMIT).toInt(Torrent::DEFAULT_SEEDING_TIME_LIMIT);
    options.inactiveSeedingTimeLimit = jsonObj.value(OPTION_INACTIVESEEDINGTIMELIMIT).toInt(Torrent::DEFAULT_SEEDING_TIME_LIMIT);
