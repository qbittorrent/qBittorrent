...
const int torznabRSSParsingResultTypeId = qRegisterMetaType<TorznabRSSParsingResult>();

void TorznabRSSParser::parse(const QString &indexerName, const QByteArray &feedData)
{
    ...
    if (xml.readNextStartElement())
    {
        ...
        if (!foundChannel)
            xml.raiseError(tr("Invalid RSS feed."));
    }
...
void TorznabRSSParser::parseRSSChannel(QXmlStreamReader &xml)
{
    while (xml.readNextStartElement())
    {
...
void TorznabRSSParser::parseRSSItem(QXmlStreamReader &xml)
{
    ...
    while (xml.readNextStartElement())
    {
        ...
        {
            searchResult.fileSize = parseInt(xml.readElementText().trimmed()).value_or(-1);
        }
        ...
        else if (name == QLatin1String("attr"))
        {
            ...
            {
                searchResult.numLeechers = parseInt(xml.attributes().value(QLatin1String("value")).toString()).value_or(-1);
            }
            else if (attrName == QLatin1String("seeders"))
            {
                searchResult.numSeeders = parseInt(xml.attributes().value(QLatin1String("value")).toString()).value_or(-1);
            }
